...
const messages = Messages.loadMessages('sfdx-easy-sources', 'translations_merge');
...
export default class Merge extends SfdxCommand {
    ...
    public async run(): Promise<AnyJson> {
        ...
        var result = await merge(this.flags, TRANSLATIONS_SUBPATH, TRANSLATIONS_EXTENSION, TRANSLATIONS_ROOT_TAG, TRANSLATION_ITEMS);
...
export async function translationMerge(options: any = {}): Promise<AnyJson> {
    ...
    const result = await merge(options, TRANSLATIONS_SUBPATH, TRANSLATIONS_EXTENSION, TRANSLATIONS_ROOT_TAG, TRANSLATION_ITEMS);
