...
export default class Split extends SfdxCommand {
    ...
    public async run(): Promise<AnyJson> {
        ...
        const inputApp = (this.flags.input) as string;

        var fileList = [];

        if (inputApp) {
            fileList = inputApp.split(',');
        } else {
        ...
        for (const filename of fileList) {
            ...
            const appProperties = xmlFileContent[APPLICATIONS_ROOT_TAG] ?? {};

            const appName = removeExtension(basename(inputFile));
            const outputDir = join(baseOutputDir, appName);

            for (const tag_section in APPLICATION_ITEMS) {

                var myjson = appProperties[tag_section];
                if (myjson == undefined) continue;

                // generate _tagId column
                generateTagId(myjson, APPLICATION_ITEMS[tag_section].key, APPLICATION_ITEMS[tag_section].headers);
            ...
            if (fs.existsSync(outputDir)) {
                const outputFileXML = join(outputDir, appName + XML_PART_EXTENSION);
